...
package object barneshut {
  ...
  case class Fork(
    ...
    val centerX
      : Float = (quads.map(_.centerX).min + quads.map(_.centerX).max) / 2f
    val centerY
      : Float = (quads.map(_.centerY).min + quads.map(_.centerY).max) / 2f
    ...
    val massX: Float = {
      if (mass == 0) centerX
        ...
        quads.foldLeft(0f) {
          case (sum, quad) => sum + quad.mass * quad.massX
        } / mass
    }
    val massY: Float = {
      if (mass == 0) centerY
        ...
        quads.foldLeft(0f) {
          case (sum, quad) => sum + quad.mass * quad.massY
        } / mass
    }
  ...
  case class Leaf(centerX: Float,
    ...
    val massX: Float = bodies.foldLeft(0f) {
      case (sum, b) => sum + b.mass * b.x
    } / mass
    val massY: Float = bodies.foldLeft(0f) {
      case (sum, b) => sum + b.mass * b.y
    } / mass
    ...
    def insert(b: Body): Quad = {
      ...
      if (size <= minimumSize) copy(bodies = newBodies)
        ...
        newBodies.foldLeft(Fork(nw, ne, sw, se))(
          (agg, body) => agg.insert(body))
      }
    }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    val matrix: Array[ConcBuffer[Body]] =
