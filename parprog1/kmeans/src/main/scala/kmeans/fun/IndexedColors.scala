...
class IndexedColorFilter(initialImage: Img,
  ...
  private def initializeIndex(numColors: Int,
                              points: GenSeq[Point]): GenSeq[Point] = {
    val initialPoints: GenSeq[Point] =
      initStrategy match {
        ...
        case UniformSampling =>
          val sep: Int = 32
          (for (r <- 0 until 255 by sep; g <- 0 until 255 by sep;
                b <- 0 until 255 by sep) yield {
        ...
        case UniformChoice =>
          ...
          for (r <- 0 until 256 by d; g <- 0 until 256 by d;
               b <- 0 until 256 by d)
            yield
  ...
  private def computeSNR(points: GenSeq[Point],
                         means: GenSeq[Point]): Double = {
    var sound = 0.0
    var noise = 0.0
  ...
  override def converged(eta: Double)(oldMeans: GenSeq[Point],
