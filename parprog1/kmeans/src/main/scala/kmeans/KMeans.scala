...
class KMeans {
  ...
  def classify(points: GenSeq[Point],
    ...
    val pointToMeans = points.groupBy(findClosest(_, means))
    means.map(mean => mean -> pointToMeans.getOrElse(mean, GenSeq())).toMap
  }
  ...
  final def kMeans(points: GenSeq[Point],
    ...
    val isConverged = converged(eta) _
    val classified = classify(points, means)
    val newMeans = update(classified, means)
    if (!isConverged(means, newMeans)) kMeans(points, newMeans, eta)
