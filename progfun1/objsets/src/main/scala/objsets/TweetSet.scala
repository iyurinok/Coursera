...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    left.filterAcc(p, newAcc) union right.filterAcc(p, newAcc) union newAcc
  }
  ...
  def mostRetweetedAcc(t: Tweet): Tweet = {
    ...
    val m = max(t, elem)
    val leftMax = left.mostRetweetedAcc(m)
    val rightMax = right.mostRetweetedAcc(m)

    max(max(leftMax, rightMax), m)
  }
