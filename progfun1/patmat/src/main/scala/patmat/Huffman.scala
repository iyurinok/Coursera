...
object Huffman {
  ...
  /**
    ...
    * }
    */
  def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy(c => c).mapValues(list => list.size).toList
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(subTree: CodeTree, bits: List[Bit]): List[Char] = subTree match {
      ...
      case _ => throw new IllegalArgumentException("Unsupported bit value: " + bits.head)
    }
    traverse(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def lookUp(tree: CodeTree)(char: Char): List[Bit] = tree match {
      ...
      case Fork(_, right, _, _) if chars(right).contains(char) => 1 :: lookUp(right)(char)
      case _ => throw new IllegalArgumentException("Unsupported symbol: " + char)
    }
  ...
  /**
    ...
    */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table.toMap.getOrElse(char, throw new IllegalArgumentException("Unsupported symbol: " + char))
