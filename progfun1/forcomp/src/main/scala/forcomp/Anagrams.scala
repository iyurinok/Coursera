...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(identity).mapValues(_.length).toList.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences.foldRight(List[Occurrences](Nil)) { case ((ch, occ), acc) => {
      acc ++ (for {comb <- acc; n <- 1 to occ} yield (ch, n) :: comb)
    }
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.toMap
    (for {(ch, n) <- x} yield (ch, n - yMap.getOrElse(ch, 0))).filter { case (_, n) => n > 0 }
  }
