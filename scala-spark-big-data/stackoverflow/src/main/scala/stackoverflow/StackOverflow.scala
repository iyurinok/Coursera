...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(
    ...
    val answers =
      postings.filter(_.postingType == 2).map(a => (a.parentId.get, a))
  ...
  def vectorPostings(
    ...
    scored
      .map(
        score =>
          (firstLangInTag(score._1.tags, langs).getOrElse(-1) * langSpread,
           score._2))
      .filter(_._1 >= 0)
      .cache()
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)],
    ...
    val clusterAvg =
      clustered.groupByKey().mapValues(averageVectors).collect().toMap
    val newMeans =
      means.indices.map(i => clusterAvg.getOrElse(i, means(i))).toArray
  ...
  def clusterResults(means: Array[(Int, Int)],
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langLabel
        : String = langs(mostCommonLang._1 / langSpread) // most common language in the cluster
      val langPercent
        : Double = (mostCommonLang._2 / vs.size.toDouble) * 100.0 // percent of the questions in the most common language
      val clusterSize: Int = vs.size

      val allScores = vs.map(_._2).toList.sorted
      val medianScore: Int = allScores.size % 2 match {
        case 0 =>
          (allScores(allScores.size / 2) + allScores(allScores.size / 2 - 1)) / 2
