...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val fields = columnNames.tail.map(field =>
      StructField(field, DoubleType, nullable = false))
  ...
  /** @return The initial data frame columns partitioned in three groups: primary needs (sleeping, eating, etc.),
    ...
    */
  def classifyColumnsRec(
      columnNames: List[String],
      primary: List[Column],
      working: List[Column],
      leisure: List[Column]): (List[Column], List[Column], List[Column]) =
    columnNames match {
      case Nil => (primary, working, leisure)

      case x :: xs =>
        if (x.startsWith("t01") || x.startsWith("t03") || x.startsWith("t11") || x
        ...
        else if (x.startsWith("t02") || x.startsWith("t04") || x.startsWith(
                   "t06") || x.startsWith("t07") || x.startsWith("t08") || x
                   .startsWith("t09") ||
                 x.startsWith("t10") || x.startsWith("t12") || x.startsWith(
                   "t13") || x.startsWith("t14") || x.startsWith("t15") || x
                   .startsWith("t16") || x.startsWith("t18"))
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column =
      when(df("telfs") < 3, "working").otherwise("not working").as("working")
    ...
    val primaryNeedsProjection: Column =
      (sum_udf(primaryNeedsColumns, lit(0.0d)) / 60.0).as("primaryNeeds")
    val workProjection: Column =
      (sum_udf(workColumns, lit(0.0d)) / 60.0d).as("work")
    val otherProjection: Column =
      (sum_udf(otherColumns, lit(0.0d)) / 60.0d).as("other")
